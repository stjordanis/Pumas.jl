...
for (i, v) in enumerate([-0.114654,0.0350263,-0.024196,-0.0870518,0.0750881,0.059033,-0.114679,-0.023992,-0.0528146,-0.00185361])
  @test PuMaS.rfx_estimate(mdsl, data[i], x0, Laplace())[1] ≈ v rtol=1e-3
end

@test PuMaS.marginal_nll_nonmem(mdsl,data,x0,[0.0],Laplace()) ≈ 57.19397077905644 rtol=1e-6
@test PuMaS.marginal_nll_nonmem(mdsl,data,x0,(η=[0.0],),Laplace()) ≈ 57.19397077905644 rtol=1e-6
@test PuMaS.marginal_nll_nonmem(mdsl,data,x0,Laplace()) ≈ 56.810343602063618 rtol=1e-6
