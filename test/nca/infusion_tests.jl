...
df.route = "inf"
data = @test_nowarn parse_ncadata(df, id=:ID, time=:Time, conc=:Prefilter_Conc, warn=false, route=:route, amt=:Amount, duration=:Infusion_Time, timeu=timeu, concu=concu, amtu=amtu)
