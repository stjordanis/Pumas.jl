...
data1 = @test_nowarn read_nca(df, id=:ID, time=:TIME, conc=:TCONC, occasion=:PERIOD, group=:METABOLITE, verbose=false, ii=0.1)
data2 = @test_nowarn read_nca(df, id=:ID, time=:TIME, conc=:TCONC, occasion=:PERIOD, group=[:METABOLITE], verbose=false)
...
pop = @test_nowarn read_nca(df, id=:ID, time=:TIME, conc=:TCONC, occasion=:PERIOD, group=[:METABOLITE], verbose=false, ii=ii1)
@test all(i -> pop[i].ii == ii1[i], eachindex(pop))
ii2 = [0.1 * i for i in 1:5]
pop = @test_nowarn read_nca(df, id=:ID, time=:TIME, conc=:TCONC, occasion=:PERIOD, group=[:METABOLITE], verbose=false, ii=ii2)
...
@test_throws ArgumentError read_nca(df, id=:ID, time=:TIME, conc=:TCONC, occasion=:PERIOD, group=[:METABOLITE], verbose=false, ii=ii3)
