...
_lpdf(d::Union{Distributions.Sampleable,Gaussian},x) = x === missing ? zval(d) : logpdf(d,x)
_lpdf(d::Union{Distributions.Sampleable,Gaussian},x::Number) = isnan(x) ? zval(d) : logpdf(d,x)
...
function rfx_estimate_dist(m::PKPDModel, subject::Subject, x0::NamedTuple, approx::Laplace, args...; kwargs...)
  ...
  Gaussian(vy0, inv(Symmetric(DiffResults.hessian(diffres))))
end
...
function FIM(m::PKPDModel, subject::Subject, x0, vy0, args...; kwargs...)
