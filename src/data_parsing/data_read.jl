...
function process_nmtran(data,cvs=Symbol[],dvs=Symbol[:dv];
  ...
  subjects = map(uids) do uid
    ## Observations
    idx_obs = filter(i -> data[id][i] == uid && data[evid][i] == 0, 1:m)
    ...
    sort!(events)
    Subject(id = uid, obs = observations, cvs = covariates, evs = events)
  end
  Population(subjects)
end
